    ...
    if (_net_print_socket < 0) {
        ...
        if (sk >= 0) {
            ...
            net_print_string(__FILE__, __LINE__,
                             "net_print_init() successful, socket=%d, testing "
                             "for hi-low-byte using int 0x12345678:\n",
                             _net_print_socket);
...
int net_print_binary(int format, const void* data, int len) {
    ...
    for (k = 0; k < len;) {
        ...
        switch (format) {
            case 'x':
                for (col = 0; col < 8 && k < len && ((len - k) / 2 > 0);
                     col++, k += 2) {
                ...
                break;
            case 'X':
                for (col = 0; col < 4 && k < len && ((len - k) / 4 > 0);
                     col++, k += 4) {
                    ret = sprintf(out, "%02X%02X%02X%02X ", *in, *(in + 1),
                                  *(in + 2), *(in + 3));
                    if (col == 1) {
                        strcat(out++, " ");
                    }
